...
import 'adddinas.dart';

class dinas extends StatefulWidget {
  final kodeunitkerja;
  final id_pegawai;
  ...
  @override
  State<dinas> createState() => _dinas();
}

class _dinas extends State<dinas> {
  ...
  getdata() async {
    ...
    if (id != null) {
      await viewCuti.getdatadinas(id, widget.kodeunitkerja);
    ...
    return Scaffold(
      ...
      body: ChangeNotifierProvider<CutiVM>(
          ...
          child: Consumer<CutiVM>(builder: (context, viewCuti, _) {
            switch (viewCuti.datadinas.status) {
              case Status.LOADING:
                return LoadingWidget();
              case Status.ERROR:
                return MyErrorWidget(viewCuti.datadinas.message ?? "NA");
              case Status.COMPLETED:
                return _widgetbody(viewCuti.datadinas.data!.data!);
              default:
            }
            return Container();
          })),
          ...
          var result = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (BuildContext context) =>
                  adddinas(id_pegawai: widget.id_pegawai),
            ),
          );
  ...
  Widget _widgetbody(List<Dinas>? Listdinas) {
    if (Listdinas!.length > 0) {
      return ListView.builder(
          itemCount: Listdinas?.length,
          itemBuilder: (context, position) {
            return _getCutiListItem(Listdinas[position]);
          });
    } else {
      return datakosong();
    }
  ...
  Widget _getCutiListItem(Dinas item) {
                    ...
                    subtitle: Container(
                      child: Column(
                          ...
                          children: [
                            ...
                            Container(
                                child: Row(
                              children: [Text(item.nospt.toString())],
                            )),
                            Text(
                              "Status :",
                              style: TextStyle(
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Center(
                    child: Text(
                      "Opps... untuk saat ini anda belum mengajukan Izin Dinas untuk mengajukan Izin Dinas anda dapat menekan tombol di bawah pojok kanan",
